    ...
    def convert_file(self, file_path: str) -> Dict[str, Any]:
        ...
        if self.llm_provider and self.config.get('use_llm', True):
            ...
            optimized_content = self.llm_provider.optimize_content(
                content_text, 
                options=self.config.get('llm_options')
            )
            ...
            if self.config.get('generate_summary', False):
                summary = self.llm_provider.generate_summary(
                    content_text,
                    max_length=self.config.get('summary_length', 100)
                )
        ...
        if not self.platform_provider.validate(converted_content):
            raise ValueError(
                "The converted content does not meet the platform requirements"
            )
            
        return converted_content
        
    def save_converted_file(self, converted_content: Dict[str, Any], output_path: str) -> None:
        """
        変換後のコンテンツをファイルに保存する
        ...
        try:
            ...
            raise IOError(f"Failed to save converted file: {e}")